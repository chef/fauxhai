...
FAUXHAI_PLAN_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FAUXHAI_REPO_ROOT="$(cd "${FAUXHAI_PLAN_DIR}/.." && pwd)"
...
do_unpack() {
  ...
  cp -RT "$FAUXHAI_REPO_ROOT" "$HAB_CACHE_SRC_PATH/$pkg_dirname/"
}
do_build() {
...
do_install() {
  ...
  if [[ -f "${FAUXHAI_REPO_ROOT}/NOTICE" ]]; then
    ...
    build_line "Warning: NOTICE not found at ${FAUXHAI_REPO_ROOT}/NOTICE"
  fi
  ...
  for binstub in "${pkg_prefix}"/bin/*; do
    sed -i "/require \"rubygems\"/r ${FAUXHAI_REPO_ROOT}/binstub_patch.rb" "$binstub"
  done
